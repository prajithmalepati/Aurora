...
## GPT 5.5.1 Handoff For Next Chat

This file is the compressed decision layer over the design-review pile. A new Claude chat should read this file first, then read `.kilo/plans/1779736434868-nimble-wolf.md` for the implementation-readiness audit, then use the current bug report as the source of truth for debugging.

### Current Decision State

- `nimble-wolf` is the implementation-readiness checklist. It has the backend/API blockers, frontend type gaps, kill-list cleanup, analyser prototype gate, and phased task breakdown.
- This `gpt5.51plan.md` file is the review synthesis. It says which Kimi/Qwen suggestions to fold, ignore, or prototype.
- Do not treat any external review as binding if it conflicts with repo rules, `CLAUDE.md`, or current code reality.
- For visual-overhaul work, preserve Aurora's existing player architecture. In particular, do not break the two-effect separation in `useAudioPlayer.ts`.

### Highest-Risk Technical Unknown

The audio analyser path is still unproven because `useAudioPlayer.ts` creates Howls with `html5: true`. Before implementing audio-reactive shader behavior, create a minimal prototype proving that amplitude data is non-zero with the actual playback path.

If `Howler.masterGain` is silent in `html5: true` mode, choose one:

- Use a `MediaElementSource` from the underlying Howler audio node if accessible and safe.
- Switch the relevant Howl path away from `html5: true` only if FLAC/large-file behavior remains acceptable.
- Drop audio-reactive curtains for v1 and keep `uAmplitude = 0`.

Do not assume this will work. Test it before shader work.

### Fold Into The Plan

- Add `PRAGMA busy_timeout=5000` per SQLite connection.
- Prefer Pillow `quantize()` for dominant color extraction instead of `scikit-learn` KMeans.
- Add canvas DPR cap: `Math.min(window.devicePixelRatio || 1, 1.5)`.
- Clamp RGB values to `[0, 1]` before passing them to WebGL uniforms.
- Handle WebGL context loss/restoration and clean up RAF/program/shader resources.
- Wrap shader time, e.g. pass `uTime = elapsedSeconds % 1000`.
- Use `highp` when available in GLSL.
- Gate decorative JS loops for reduced motion, not only CSS visibility.
- Use refs/throttling for high-frequency values: analyser amplitude, idle state, shader uniforms.
- Use stable SVG IDs if clip paths are used; no random IDs during render.
- Use a native invisible `<input type="range">` overlay for waveform seeking and accessibility.
- Use SVG `viewBox` for waveform geometry so path generation does not depend on reading layout during render.

### Skip For Now

- Ray marching / volumetric aurora.
- Seasonal hue rotation.
- Magnetic wind from album art.
- Seismograph waveform layout.
- Ghost waveform that pulses to analyser while peaks load.
- Tailwind `@theme` migration as a blocker.
- Custom ARIA SVG slider.
- Mandatory subprocess isolation for `miniaudio` in v1.

### Prototype Before Locking

- Analyser with `html5: true`.
- Additive shader blending.
- Liquid-glass blend-mode/refraction tricks.
- Whether the play-button interior star should stay or be reduced to bloom/specular only.

### Debugging Guidance For New Chat

For the upcoming bug-fix chat, do not start visual-overhaul implementation unless explicitly asked. Debug the reported issue first, read the relevant files, reproduce or inspect the failure, then make the smallest fix. Use this file only as context if the bug touches the visual overhaul, audio player, shader, scanner, waveform, or related planning docs.
