...
fn build_test_app() -> (axum::Router, Arc<AppState>) {
    ...
    seed_database(&conn);
    let state = Arc::new(AppState { conn: Mutex::new(conn), db_path: None, addon_state: Arc::new(aurora_server::routes::addons::AddonState::new()) });
...
async fn stream_range_tests() {
    ...
    let state = Arc::new(AppState { conn: tokio::sync::Mutex::new(conn), db_path: None, addon_state: Arc::new(aurora_server::routes::addons::AddonState::new()) });
